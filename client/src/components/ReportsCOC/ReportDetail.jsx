...
const ReportDetailCOC = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(assignment => {
        ...
        if (searchField === 'instructor') {
          ...
          return (
            (assignment.instructorId.fullName || '').toLowerCase().includes(searchLower) ||
            (assignment.instructorId.chair || '').toLowerCase().includes(searchLower) ||
    ...
    if (sortConfig.key) {
      filtered.sort((a, b) => {
        ...
        if (sortConfig.key === 'instructor') {
          ...
          aValue = a.preferenceRank !== undefined ? a.preferenceRank : 999;
          bValue = b.preferenceRank !== undefined ? b.preferenceRank : 999;
  ...
  const getAssignmentsCsvData = () => {
    ...
    return allAssignments.map(assignment => ({
      ...
      'Preference Rank': assignment.preferenceRank !== undefined ? assignment.preferenceRank : 'N/A',
      'Experience Years': assignment.experienceYears !== undefined ? assignment.experienceYears : 'N/A',
        ...
        try {
          ...
          const backgroundColors = isDarkMode 
            ? ['rgba(99, 102, 241, 0.85)', 'rgba(79, 70, 229, 0.85)', 'rgba(67, 56, 202, 0.85)', 
               'rgba(55, 48, 163, 0.85)', 'rgba(49, 46, 129, 0.85)']
            : ['rgba(99, 102, 241, 0.85)', 'rgba(129, 140, 248, 0.85)', 'rgba(165, 180, 252, 0.85)', 
               'rgba(196, 181, 253, 0.85)', 'rgba(224, 231, 255, 0.85)'];
  ...
  // Render assignment reasoning details
  const renderAssignmentReason = (assignment) => {
    // Check if any reasoning data exists
    ...
    if (!hasReasoningData) {
      return (
        <div className="text-center py-3 text-sm text-gray-500 dark:text-gray-400">
          No assignment reasoning data available
        </div>
      );
    }
      ...
      <div className="p-4 space-y-4">
        {assignment.assignmentReason && (
          <div className="bg-indigo-50 dark:bg-indigo-900/20 p-3 rounded-lg border border-indigo-100 dark:border-indigo-800/30">
            <div className="flex items-start">
              <Sparkles className="h-4 w-4 text-indigo-600 dark:text-indigo-400 mt-0.5 mr-2 flex-shrink-0" />
              <div>
                <div className="text-xs font-medium text-indigo-800 dark:text-indigo-300 mb-1">Assignment Reason</div>
                <p className="text-sm text-indigo-700 dark:text-indigo-200">{assignment.assignmentReason}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {assignment.preferenceRank !== undefined && (
            <div className="bg-white dark:bg-gray-700 rounded-lg p-3 shadow-sm border border-gray-200 dark:border-gray-600">
              <div className="flex items-center">
                <Star className="h-4 w-4 text-amber-500 dark:text-amber-400 mr-1.5" />
                <span className="text-xs font-medium text-gray-600 dark:text-gray-300">Instructor Preference</span>
              </div>
              <div className="mt-1 flex items-center">
                ...
                {assignment.preferenceRank > 0 && assignment.preferenceRank <= 3 && (
                  <span className="ml-2 px-2 py-0.5 text-xs rounded-full bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300">
                    High Preference
                  </span>
                )}
              </div>
          ...
          {assignment.experienceYears !== undefined && (
            <div className="bg-white dark:bg-gray-700 rounded-lg p-3 shadow-sm border border-gray-200 dark:border-gray-600">
              <div className="flex items-center">
                <Clock3 className="h-4 w-4 text-blue-500 dark:text-blue-400 mr-1.5" />
                <span className="text-xs font-medium text-gray-600 dark:text-gray-300">Teaching Experience</span>
              </div>
              <div className="mt-1">
                <span className="text-base font-semibold text-gray-800 dark:text-white">
                  {assignment.experienceYears} {assignment.experienceYears === 1 ? 'Year' : 'Years'}
                </span>
                {assignment.experienceYears >= 3 && (
          ...
          {assignment.score !== undefined && (
            <div className="bg-white dark:bg-gray-700 rounded-lg p-3 shadow-sm border border-gray-200 dark:border-gray-600">
              <div className="flex items-center">
                <Award className="h-4 w-4 text-purple-500 dark:text-purple-400 mr-1.5" />
                <span className="text-xs font-medium text-gray-600 dark:text-gray-300">Assignment Score</span>
              </div>
              <div className="mt-1">
                <span className="text-base font-semibold text-gray-800 dark:text-white">
                  {typeof assignment.score === 'number' ? assignment.score.toFixed(2) : assignment.score}
                </span>
                {assignment.score >= 0.8 && (
                  <span className="ml-2 px-2 py-0.5 text-xs rounded-full bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300">
                    Strong Match
                  </span>
                )}
              </div>
          ...
          <div className="relative">
            <div className="w-16 h-16 mx-auto mb-3">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <Loader2 className="text-indigo-600 dark:text-indigo-400 h-8 w-8 animate-pulse" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 dark:border-red-700 p-5 rounded-lg shadow">
            ...
            <div className="mt-4">
              <button
                ...
                className="inline-flex items-center px-4 py-2 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 rounded-md hover:bg-red-200 dark:hover:bg-red-900/50 transition-colors"
              >
                Retry
              </button>
            </div>
      ...
      <div className="bg-white dark:bg-gray-800 shadow-sm rounded-2xl border border-gray-200 dark:border-gray-700 p-4 mb-4">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
          <div className="relative flex-grow w-full md:w-auto max-w-md">
            ...
            <input
              type="text"
              placeholder={`Search by ${searchField === 'instructor' ? 'instructor name' : searchField === 'chair' ? 'chair' : 'any field'}`}
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="block w-full pl-10 pr-10 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-base shadow-sm"
              aria-label="Search"
            />
            {searchTerm && (
              <button
          ...
          <div className="w-full md:w-auto flex flex-wrap gap-2">
            ...
            <button
              ...
              className={`inline-flex items-center px-3 py-2.5 border shadow-sm text-sm font-medium rounded-xl ${
                showFilters 
                  ? 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 border-indigo-300 dark:border-indigo-700'
                  : 'text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 border-gray-300 dark:border-gray-600'
            ...
            <a 
              ...
              className="inline-flex items-center px-3 py-2.5 border border-transparent shadow-sm text-sm font-medium rounded-xl text-white bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-700 dark:hover:bg-indigo-600 focus:outline-none transition-colors"
            >
            ...
            >
              <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="flex flex-col">
                  ...
                  <select
                    ...
                    className="mt-1 block w-full py-2.5 px-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 rounded-xl shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 text-base transition-colors"
                  >
                  ...
                  >
                    ...
                    {activeTab === 'instructors' ? (
                      ...
                      <>
                        ...
                        <option value="score_descending">Score (High-Low)</option>
                        <option value="preferenceRank_ascending">Preference Rank (Low-High)</option>
                        <option value="experienceYears_descending">Experience (High-Low)</option>
                      </>
                    )}
                  </select>
                </div>
              ...
              <div className="mt-4 flex justify-end">
                <button
                  ...
                  className="px-4 py-2 text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 focus:outline-none transition-colors"
                >
                  Reset All Filters
                </button>
              </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
            ...
            {searchTerm && (
              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-900/40 text-indigo-800 dark:text-indigo-200">
                {searchField === 'instructor' ? 'Instructor' : searchField === 'chair' ? 'Chair' : 'Search'}: {searchTerm}
                <button
                  onClick={() => setSearchTerm('')}
            ...
            {sortConfig.key && (
              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full bg-pink-100 dark:bg-pink-900/40 text-pink-800 dark:text-pink-200">
                ...
                >
                  <X className="h-3.5 w-3.5 text-pink-600 dark:text-pink-400" />
                </button>
              </span>
            )}
            ...
            <button
              onClick={resetFilters}
              className="text-xs underline text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 transition-colors"
            >
              Clear All
            </button>
          </div>
          ...
          <div className="max-w-7xl mx-auto px-4 py-5">
            <div className="flex justify-between items-center">
              <div className="flex-1 min-w-0">
                <div className="flex items-center">
                  <div className="bg-gradient-to-r from-indigo-500 to-purple-500 p-3 rounded-xl text-white shadow-sm mr-4 hidden sm:flex">
                  ...
                  <div>
                    <h1 className="text-xl md:text-2xl font-bold leading-tight text-gray-900 dark:text-white flex items-center gap-2">
                      <span className="sm:hidden">
                        <FileBarChart2 className="text-indigo-600 dark:text-indigo-400 h-5 w-5 md:h-6 md:w-6 mr-2" />
                      </span>
              ...
              <div className="hidden md:flex md:items-center md:space-x-3">
                <div className="h-8 border-l border-gray-300 dark:border-gray-600 mx-3 hidden sm:block"></div>
                
                <Link to={`/reports/${id}/edit`}>
                  <button className="inline-flex items-center px-4 py-2.5 bg-white dark:bg-gray-700 text-indigo-600 dark:text-indigo-400 border border-indigo-600 dark:border-indigo-400 rounded-xl hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition shadow-sm text-sm font-medium">
                ...
                <button
                  ...
                  className="inline-flex items-center px-4 py-2.5 border border-transparent rounded-xl shadow-sm text-white bg-red-600 hover:bg-red-700 dark:bg-red-700 dark:hover:bg-red-600 focus:outline-none transition text-sm font-medium"
                >
              ...
              <div className="md:hidden">
                <button 
                  ...
                  className="inline-flex items-center justify-center p-2 rounded-lg text-gray-400 hover:text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors"
            ...
            <AnimatePresence>
              {mobileMenuOpen && (
                <motion.div 
                  ...
                  className="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700 absolute right-4 z-20 mt-2 w-48 overflow-hidden"
                >
                  <div className="py-1">
                    <Link 
                      to={`/reports/${id}/edit`} 
              ...
              {confirmDelete && (
                ...
                >
                  <motion.div 
                    ...
                    transition={{ duration: 0.2 }}
                    className="bg-white dark:bg-gray-800 rounded-2xl max-w-md w-full p-6 shadow-md"
                  >
                    ...
                    <h3 className="text-xl font-medium text-center text-gray-900 dark:text-white mb-2">Delete Report</h3>
                    ...
                    <div className="flex justify-center space-x-4">
                      <button
                        ...
                        className="px-4 py-2.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors font-medium"
                      >
                        Cancel
                      </button>
                      <button
                        ...
                        disabled={deleting}
                        className="px-4 py-2.5 bg-red-600 hover:bg-red-700 dark:bg-red-700 dark:hover:bg-red-600 text-white rounded-xl transition-colors font-medium flex items-center"
                      >
                        {deleting ? (
                          <>
        ...
        {activeTab === 'overview' && (
          ...
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              >
                <div className="flex-shrink-0 bg-indigo-100 dark:bg-indigo-900/50 rounded-xl p-3 shadow-sm">
              ...
              >
                <div className="flex-shrink-0 bg-blue-100 dark:bg-blue-900/50 rounded-xl p-3 shadow-sm">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">{uniqueCourses}</p>
                </div>
              </motion.div>
              ...
              >
                <div className="flex-shrink-0 bg-green-100 dark:bg-green-900/50 rounded-xl p-3 shadow-sm">
              ...
              >
                <div className="flex-shrink-0 bg-purple-100 dark:bg-purple-900/50 rounded-xl p-3 shadow-sm">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">{formatNumber(totalWorkload)}</p>
                </div>
              </motion.div>
            </div>
              ...
              >
                ...
                <div className="px-5 py-4">
                  <dl className="space-y-3">
                    <div className="flex justify-between py-2.5 border-b border-gray-100 dark:border-gray-700">
                    ...
                    <div className="flex justify-between py-2.5 border-b border-gray-100 dark:border-gray-700">
                      <dt className="text-sm font-medium text-gray-500 dark:text-gray-400 flex items-center">
                        <div className="p-1.5 rounded-md bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 mr-3">
                    ...
                    <div className="flex justify-between py-2.5">
                      <dt className="text-sm font-medium text-gray-500 dark:text-gray-400 flex items-center">
                        <div className="p-1.5 rounded-md bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 mr-3">
                          <Clock className="h-4 w-4" />
                        </div>
                        Last Updated
                      </dt>
                  ...
                  {report.note && (
                    <div className="mt-5 pt-4 border-t border-gray-200 dark:border-gray-700">
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400 p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border border-indigo-100 dark:border-indigo-800/30">
                        {report.note}
                      </p>
                    </div>
                  )}
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div className="flex items-center space-x-3">
                          <div className="flex-shrink-0">
                            <div className="h-10 w-10 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white font-medium text-lg shadow-sm">
                    ...
                    <div className="md:hidden px-5 py-4 space-y-3 divide-y divide-gray-200 dark:divide-gray-700">
                      {assignments.map((assignment, idx) => (
                        <div key={idx} className="pt-3 first:pt-0">
                          ...
                          <button
                            onClick={() => toggleItemExpansion(`instructor_${instructor._id}_assignment_${idx}`)}
                            className="mt-2 text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 flex items-center"
                          >
                            {isItemExpanded(`instructor_${instructor._id}_assignment_${idx}`) ? (
                              <>
                      ...
                      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-700">
                          <tr>
                            ...
                            <th scope="col" className="px-4 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider whitespace-nowrap">Details</th>
                          </tr>
                        </thead>
                              ...
                              <tr className={`hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${
                                ...
                                <td className="px-4 py-3 whitespace-nowrap text-center">
                                  <button
                                    ...
                                    aria-label={isItemExpanded(`instructor_${instructor._id}_assignment_${idx}`) ? "Hide details" : "Show details"}
                                  >
                                    {isItemExpanded(`instructor_${instructor._id}_assignment_${idx}`) ? (
                                      <ChevronUp size={18} />
                                    ) : (
                              ...
                              <AnimatePresence>
                                {isItemExpanded(`instructor_${instructor._id}_assignment_${idx}`) && (
                                  <motion.tr
                                    ...
                                    transition={{ duration: 0.2 }}
                                    className="bg-indigo-50/50 dark:bg-indigo-900/5"
                                  >
                  ...
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
                      {paginatedAssignments.map((assignment, idx) => (
                        <React.Fragment key={idx}>
                          <tr className={`hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${
                            isItemExpanded(`assignment_${idx}`) 
                            ? "bg-indigo-50/50 dark:bg-indigo-900/5" : ""
                          }`}>
                            <td className="px-4 py-3 whitespace-nowrap">
                              <div className="flex items-center">
                            ...
                            <td className="px-4 py-3 whitespace-nowrap">
                              <div className="flex items-center">
                                <div className="flex-shrink-0 h-6 w-6 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 flex items-center justify-center">
                            ...
                            <td className="px-4 py-3 whitespace-nowrap text-center">
                              <button
                                onClick={() => toggleItemExpansion(`assignment_${idx}`)}
                                className={`p-1.5 rounded-lg ${
                                  isItemExpanded(`assignment_${idx}`)
                                    ? "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300" 
                                    : "hover:bg-gray-100 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 dark:hover:bg-gray-700"
                                } transition-colors`}
                                aria-label={isItemExpanded(`assignment_${idx}`) ? "Hide details" : "Show details"}
                              >
                                {isItemExpanded(`assignment_${idx}`) ? (
                                  <ChevronUp size={18} />
                                ) : (
                      ...
                      <tr className="bg-gray-50 dark:bg-gray-700">
                        <td colSpan="5" className="px-4 py-3 whitespace-nowrap text-right text-sm font-medium text-gray-700 dark:text-gray-300">
                          Total Workload:
                        </td>
